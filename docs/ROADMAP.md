...
### AC-020 — Decide repository license and native-provider commercial boundaries

Public visibility is requested, but no software license or right to resell provider usage has been chosen. Monetization and SaaS remain possibilities.

**Outcome:** Prepare the owner decision on licensing and future customer-owned-runner/hosted coordination boundaries, using primary provider terms.

**Acceptance:**

- Compare the licensing choices against the owner's goals; do not silently install MIT/AGPL/commercial terms.
- Obtain an explicit owner decision before marking the project open source or accepting outside code under a license.
