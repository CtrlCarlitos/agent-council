...
// conversion), recorded through the production contract. The adopted lease
// keeps the fixture's lease value so existing command call sites remain
// valid.
func adoptControllerForTest(t *testing.T, store *storage.Store, runID, lease string) {
	t.Helper()
