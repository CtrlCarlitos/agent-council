...
// POSIX rollout file integrity for the codex adapter (AC-009 spec §3.7):
// the bounded UUID-suffix scan, path-integrity checks, and the baseline
// scan with POSIX ownership/mode identity. The dev:ino file identity and
// ownership checks are POSIX-only; Windows is the fail-closed platform
// statement (rollout_windows.go, §3.7 honest gap).
...
// verifyRolloutOwnership enforces the §3.7 POSIX ownership/mode checks:
// the rollout is owned by the current user with no group or other
// access (the 0600 family — the native process writes and appends it
// privately, so anything broader is integrity evidence against the
// protected class).
func verifyRolloutOwnership(fi os.FileInfo, path string) error {
	st, ok := fi.Sys().(*syscall.Stat_t)
	if !ok {
		return fmt.Errorf("rollout %s ownership is not verifiable on this platform", path)
	}
	if int(st.Uid) != os.Getuid() {
		return fmt.Errorf("rollout %s is owned by uid %d, not the current user %d", path, st.Uid, os.Getuid())
	}
	if fi.Mode().Perm()&0o077 != 0 {
		return fmt.Errorf("rollout %s must have no group/other access, got %o", path, fi.Mode().Perm())
	}
	return nil
}
...
// the session_meta line carrying session_id == native id. POSIX
// ownership/mode identity is enforced. A torn final line is ignored; a
// mid-file parse failure invalidates the read.
func scanRolloutBaseline(path, nativeID string) (identity string, size int64, entries int, err error) {
	...
	if err := verifyRolloutOwnership(fi, path); err != nil {
		return "", 0, 0, err
	}
